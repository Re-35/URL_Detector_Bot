...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_languages:
        # New user, ask for language with welcome message
...
• استخدم دائماً حكمك الشخصي عند النقر على الروابط
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_for == 'single_url':
        ...
        # Show main menu if no specific action is expected
        if lang == 'ar':
            text = "🔍 *فاحص الروابط*\n\nاحم بياناتك من الروابط المشبوهة\n\nاختر الخدمة المطلوبة:"
        else:
            text = "🔍 *URL Detector*\n\nProtect your data from malicious URLs\n\nChoose the service you need:"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_for == 'csv_file':
        ...
        try:
            ...
            if plot1:
                if lang == 'ar':
                    await update.message.reply_photo(plot1, caption="📊 نسبة الروابط الآمنة والمشبوهة")
                else:
                    await update.message.reply_photo(plot1, caption="📊 Distribution of Safe and Suspicious URLs")
            
            if plot2:
                if lang == 'ar':
                    await update.message.reply_photo(plot2, caption="🌍 توزيع الروابط حسب الدول")
                else:
                    await update.message.reply_photo(plot2, caption="🌍 URL Distribution by Countries")
            
            context.user_data['waiting_for'] = None
            
            # Show main menu
            if lang == 'ar':
                text = "تم الانتهاء من تحليل الملف! هل تريد فحص ملف آخر؟"
            else:
                text = "File analysis completed! Would you like to check another file?"
...
def main():
    ...
    # For Koyeb, use polling (simpler setup)
